...
type config struct {
	...
	OutputAdGuard fileOutput
	OutputResult  fileOutput
}
...
func main() {
	...
	removed := removeOverlaps(denyList, allowList)

	fmt.Printf("%d allow rules\n", len(allowList))
	fmt.Printf("%d deny rules\n", len(denyList))
	if removed > 0 {
		fmt.Printf("Removed %d deny rules that also appear in allow list\n", removed)
	}
	...
	if cfg.OutputResult.Enable {
		fmt.Printf("Output final result to %s\n", cfg.OutputResult.File)
		if err := writeLines(cfg.OutputResult.File, denyRules); err != nil {
			fail(err)
		}
	}
...
func applyConfigValue(cfg *config, section, key, value string) error {
	...
	case "outputAdGuard":
		return applyFileOutputValue(&cfg.OutputAdGuard, section, key, value)
	case "outputResult":
		return applyFileOutputValue(&cfg.OutputResult, section, key, value)
	default:
...
func applyFileOutputValue(output *fileOutput, section, key, value string) error {
	switch key {
	case "enable":
